...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for prop in properties.iterator():  # Using iterator for large dataset handling
            for i in range(3):  # Add 3 reports per property
                ...
                expiry_months = relativedelta(due_date, timezone.now().date()).years * 12 + relativedelta(due_date, timezone.now().date()).months

                document = self.create_sample_document(prop.id, f'report_{i + 1}.txt')
                ...
                try:
                    ...
                    self.stdout.write(self.style.SUCCESS(f'Added 3 Gas Safety reports for property ID: {prop.id}'))
                except Exception as e:
                    self.stdout.write(self.style.ERROR(f'Failed to create Gas Safety report for property ID: {prop.id}. Error: {e}'))
    ...
    def create_sample_document(self, property_id, filename):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f'Error writing file {filename} for property ID: {property_id}. Error: {e}'))
